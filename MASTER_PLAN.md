...
> Plan integral de mejoras, organizado en oleadas (waves) con commits atómicos y pushes individuales.
>
> Cada wave termina con `git commit` + `git push` contra `main`.
>
> Fecha de inicio: 2026-07-16 | Versión actual: **1.8.0** | Versión objetivo: **1.8.0** (Wave 5) luego **1.8.x** (Wave 6)

---

## Wave 0 — Preparación y baseline (COMPLETED)
...
## Wave 1 — Bugfixes de code quality (COMPLETED)
...
## Wave 2 — Cobertura de tests: módulos críticos (COMPLETED)

| # | Módulo | Cobertura actual → target | Archivos | Commit msg |
|---|---|---|---|---|
...
## Wave 3 — Integración de Phase 3: DLP pipeline en gateway (COMPLETED)
...
| 3.6 | Tests de integración gateway+DLP+guardrails+token | tests/test_gateway.py | `test: integration tests for DLP+guardrails+token chain in gateway` |

**Pushes**: ✅ 6/6

---

## Wave 4 — Token Traffic Monitor Dashboard (COMPLETED)
...
| 4.2 | Dashboard en UI: panel token in/out, límites, gasto | ai_blocker/ui.py | `feat: add token usage dashboard panel to GUI with real-time stats` |
...
| Wave | Commits | Descripción | Estado |
|:----:|:-------:|-------------|:------:|
...
Cada wave tiene un checkpoint de verificación antes de avanzar.
