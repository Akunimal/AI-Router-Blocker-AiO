...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def _apply_dlp(self, data: bytes, domain: str, path: str, method: str) -> bytes:
        ...
        try:
            ...
            _scan_duration_ms = int((time.monotonic() - _scan_start) * 1000)
            if findings:
                _finding_dicts = [f.to_dict() for f in findings]
                if policy.action == DLPAction.BLOCK:
                    if isinstance(dlp, DLPEngine):
                        dlp.metrics.blocked_count += 1
                    self._log_audit(
                        domain, path, method, "blocked",
                        metadata={"dlp_findings": len(findings), "policy": "block", "scan_duration_ms": _scan_duration_ms},
                        dlp_findings=_finding_dicts,
                        body=text,
                    )
                    return b""
                ...
                self._log_audit(
                    ...
                    metadata={"dlp_findings": len(findings), "scan_duration_ms": _scan_duration_ms},
                    dlp_findings=_finding_dicts,
                    body=text,
                )
    ...
    def _log_audit(self, domain: str, path: str, method: str, action: str,
        ...
        try:
            entry = AuditEntry(
                ...
                request_hash=audit.compute_hash(body) if body else "",
            )
